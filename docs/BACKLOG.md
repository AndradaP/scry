...
- [ ] Evals — run 10-15 teardowns on well-known products, score against Master Rubric. Check: are citations accurate, is Lenny's Lens using Lenny's voice only, are sections appropriately scoped?
  **Status as of 2026-09-06** (supersedes the 2026-08-29 note below, kept for history): Phase 2 (citation verification + pairwise Claude+Gemini judging) is built and has real data across all 12 products, but **do not treat any current number as final** — full detail and the exact next steps in `docs/scry-eval-status.md`'s "September 2026 update" section. Short version: a real cost incident ($62.27 in one day from unbounded claim volume) got root-caused and fixed with a hard spend ceiling, a claim cap, and batched fact-checking. The insight-scoring dimension was renamed `grounded_insight_value` and redesigned (validity is now checked against real evidence, not judge-guessed) after reading real losing rows surfaced two rubric bugs, both fixed. A parallel investigation then found the citation-verification pipeline itself had two real, now-fixed bugs (a speaker-misattribution bug, a decimal-number regex bug) — but also found a genuine ceiling on the plain word-overlap archive matcher: a real, reproducible case where a correct match (0.43 confidence) scores lower than a wrong one (0.538), which no threshold can separate. That's the concrete, evidence-based trigger for the semantic-search decision below, not an assumption. An unknown-product query fix (bootstrapping archive queries from web-search-discovered context for zero-coverage products) is built and deployed live to production but not yet merged to `main`, pending a retest against a fixed matcher. Citation precision currently sits around 51-77% depending on tier, versus a 90% gate set before any of this ran — the gate has not been cleared.
  **Status as of 2026-08-29** (history): the raw 3-arm comparison is live — `scripts/eval/run-eval.mjs` runs `scry` / `claude_vanilla` / `claude_web` for the same product and writes each to `eval_runs`. 12 products × 3 arms = 36 rows landed (8 well-known: Notion, Slack, Figma, Linear, Duolingo, Loom, Calendly, Webflow; 4 lesser-known/zero-coverage: Nuvo, Arena Physica, Resolve AI, AirOps — chosen to span the coverage spectrum). **Judging is not wired up yet** — `scripts/eval/judge-prompts.mjs` has prompt templates only (framework application depth, competitive positioning depth, insight novelty, coherence/actionability — a separate eval-specific rubric, not the Master Rubric above), `scripts/eval/orchestration-design.md` is pseudocode ("Nothing here is wired up or executable"), and `eval_judgments`/`eval_citations` are staged in `pending_eval_judgments.sql`, explicitly not yet promoted to a real migration. No Gemini API call exists anywhere in the codebase despite `judge-prompts.mjs`'s header comment describing Claude+Gemini as dual judges — don't represent judged scoring as running until `run-judgments.mjs` actually exists.
- [ ] **Semantic/lexical-matching upgrade for archive lookup** — evidence-based, not assumed: a real, reproducible case exists where plain word-overlap matching scores a correct archive match (0.43) lower than a wrong one (0.538), and no threshold or window size separates them. Try weighting distinctive/rare words over generic ones (TF-IDF style, still lexical, no new infra) first; if that doesn't resolve it, that's the real trigger for semantic embeddings/`pgvector` (schema-ready, unused). Details in `docs/scry-eval-status.md`.
- [ ] **Real web-citation content verification** — currently format-checked only (`classifyWebCitation` in `scripts/eval/run-citation-verification.mjs`), inflating precision on any product leaning on web citations. `scripts/eval/lib/web-fact-check.mjs`'s batched fact-checker already exists for this exact purpose (built for `grounded_insight_value`) — mostly wiring, not new design.
- [ ] **Generation-side fix: generic "recognizable expert" name-dropping** — `generate-teardown` names plausible real archive guests from general topic knowledge rather than what was actually retrieved for the specific product, confirmed on Webflow/Calendly/Loom. Real defect, not just an eval-measurement artifact. Not yet scoped.
- [ ] **Swap eval judge calls to a cheap open-weight model** — the tool-free calls (claim extraction, usefulness scoring, the three pairwise comparison dimensions) are the bulk of eval token volume and cleanly swappable via any inference provider; real cost reduction plus a genuine third judge family (strengthens the existing anti-self-preference-bias design, not just cheaper). Does not cleanly cover the live web fact-check (rides on Claude's built-in search tool) — would need real search wired up manually. Not scoped yet.
...
### Infrastructure
- [ ] **Security: `generate-teardown` trusts unverified JWTs** — `getUserFromJwt` only base64-decodes the token payload, it never verifies the signature. Any 3-part token with an arbitrary `email`/`sub` claim is accepted (this is exactly what the eval script's dev-bypass JWT relies on, see `scripts/eval/run-eval.mjs`). Need to check whether the function trusts the token's claimed `user_id` for writes or rate-limit checks without confirming it came from a real Supabase session — if so, anyone can forge a token to write as another user or dodge rate limits. Must fix (verify signature against Supabase's JWT secret/JWKS, or drop `verify_jwt = false` in `supabase/config.toml` for this function) before any wider beta release. Flagged during eval work on 2026-08-23, not fixed yet — do not treat as resolved. Note: fixing this will break the eval script's current auth method (the dev-bypass JWT), so `run-eval.mjs` will need a real replacement (most likely the Supabase service role key directly) when this gets fixed.
- [ ] **Silent citation-scrubbing substitution** — when `verifyParsedCitations` rejects a citation, the attributed name gets replaced with generic "the team" in the prose, with no indication to the reader that grounding was removed. Confirmed live in production-representative output (Loom, Aug 29 eval batch, `growth_model` section: "the team shared in the archive that Loom deliberately delayed monetization"). Not theoretical — happening in real generations right now. Same failure shape as the LennyData 401 that started this week's investigation: a real problem degrading silently instead of surfacing. Needs a decision: either mark visibly when this fires (even just internally, for monitoring), or reconsider whether silent fallback is the right behavior at all versus dropping the sentence entirely rather than misattributing it to an anonymous "team." Flagged during Aug 29-30 eval work — same triage tier as the JWT item above, not urgent-fix-tonight.
