    ...
    setTimeout(() => {
      ...
      CRITIQUE_SECTIONS.forEach((s) => {
        if (s.key === "lennys_lens") {
          mock[s.key] = `Lenny's take, drawing on conversations with Maggie Crowley and Gokul Rajaram: Your teardown demonstrates a solid grasp of the product's surface-level mechanics, but misses the deeper strategic layer that separates good analysis from great analysis. (Maggie Crowley, Product Leadership) The strongest teardowns connect individual feature decisions to the company's broader competitive moat — something Gokul Rajaram emphasizes in his frameworks for product evaluation. (Gokul Rajaram, Product Strategy)`;
        } else {
          mock[s.key] = `This is the ${s.label.toLowerCase()} of your teardown${productName ? ` for ${productName}` : ""}. In the full implementation, this would contain 2-4 paragraphs of specific, constructive critique generated by AI. (Jackie Bavaro, Product Management) Each piece of feedback is grounded in established product frameworks and attributed to specific domain experts. (Lenny Rachitsky, Product Strategy)`;
        }
      });
      setCritique(mock);
      setIsLoading(false);
